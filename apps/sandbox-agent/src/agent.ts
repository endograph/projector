...
import {
  ...
  type SandboxAttachmentData,
  type StoredSandboxAttachmentData,
} from "./attachments.js";
import {
  createSandboxCharter,
  getAgentControlsState,
  hydrateSandboxInstance,
  type CameraSensorImage,
  type SandboxMessage,
} from "./projector-sandbox.js";
import { attachCameraSampler, type CameraSamplerHandle } from "./vision.js";

const sandboxApiModule = "@projectors/sandbox/convex/_generated/api.js";
const { api } = (await import(sandboxApiModule)) as any;

const AGENT_NAME = "sandbox-agent";
...
const MESSAGE_TOPIC = "sandbox.message.v1";
const COMMAND_RPC_METHOD = "sandbox.command.v1";
...
type SerializedInstance = Parameters<typeof hydrateSandboxInstance>[0];
...
class SandboxVoiceAgent extends voice.Agent {
  constructor() {
    super({
      instructions: "Initializing projector sandbox voice agent.",
    });
  }
}
...
export default defineAgent({
  entry: async (ctx: JobContext) => {
    console.log("[sandbox-agent] entry starting");
    ...
    console.log(`[sandbox-agent] connected to room ${roomName}`);

    const convex = new ConvexClient(process.env.CONVEX_URL!);
    const workerId = `sandbox-agent:${process.pid}:${crypto.randomUUID()}`;
    ...
    const releaseLease = () => {
      ...
      void convex.mutation(api.livekitAgent.releaseAgentWorkerLease, {
        ...
        console.warn("[sandbox-agent] failed to release worker lease", error);
      });
    };
    ...
    const stopForLostLease = async () => {
      ...
      console.warn(`[sandbox-agent] worker lease lost for room ${roomName}; disconnecting`);
      releaseLease();
    ...
    if (!claimedLease) {
      console.warn(`[sandbox-agent] another worker owns room ${roomName}; exiting`);
    ...
    leaseSubscription = convex.onUpdate(
      ...
      (error) => {
        console.error("[sandbox-agent] worker lease subscription failed", error);
        void stopForLostLease();
      },
    );
    leaseHeartbeat = setInterval(() => {
      void renewLease().catch((error) => {
        console.error("[sandbox-agent] failed to renew worker lease", error);
        void stopForLostLease();
      });
    }, WORKER_LEASE_HEARTBEAT_MS);
    ctx.room.once(RoomEvent.Disconnected, releaseLease);
    console.log(`[sandbox-agent] claimed worker lease ${workerId} for room ${roomName}`);

    try {
      const init = (await convex.query(api.livekitAgent.getAgentInit, { roomName })) as AgentInit;
      if (!init) {
        throw new Error(`No sandbox session is associated with LiveKit room "${roomName}"`);
      }
      console.log(`[sandbox-agent] loaded session ${init.sessionId} with ${init.messages.length} messages`);

      let root = hydrateSandboxInstance(init.instance, init.sessionId);
      let referenceFrameId = init.frameId;
      const contextFrames = (await convex.query(api.sessions.listMachineContextFrames, {
        sessionId: init.sessionId,
      })) as Frame<any>[];
      console.log(`[sandbox-agent] initialized machine with ${contextFrames.length} context frame(s)`);
      ...
      const addSandboxMessage = (args: {
        role: "user" | "assistant";
        content: string;
        attachments?: StoredSandboxAttachmentData[];
        frameId?: Id<"frames">;
        mode: "text" | "voice";
      ...
      const persistStreamingMessageUpdate = (
        ...
        // Barge-in partials stay in the frame log as interrupted context, but
        // are not durable transcript messages (the executor owns that split).
        if (update.streamState === "cancelled") return;
        if (role === "assistant" && !shouldPersistAssistantStreamingUpdate(update)) return;
        return addSandboxMessage({
          role,
          content: update.text,
          mode,
      ...
      const agentDiscreteExecutor = new AiSdkExecutor<SandboxAttachmentData>({
      ...
      const agent = new SandboxVoiceAgent();
      const liveKitExecutor = new LiveKitRealtimeExecutor({
        ...
        input: {
          ...
          parseDataMessage: (payload) => parseLiveKitSandboxMessage(payload),
        },
        eventNames: {
      ...
      ctx.room.once(RoomEvent.Disconnected, () => {
        ...
        void stopCameraSampler().catch((error) => {
          console.warn("[sandbox-agent] failed to stop camera sampler", error);
        });
      });
      const createSandboxMachine = (frames: Frame<any>[]) =>
        createMachine({
          id: init.sessionId,
          instance: root,
          charter: createSandboxCharter({ cameraSensor }),
          executor: liveKitExecutor,
          frames,
        });
      let machine = createSandboxMachine(contextFrames);
      ...
      const refreshDurableMachineState = async (reason: string) => {
        ...
        root = hydrateSandboxInstance(refreshed.instance, init.sessionId);
      ...
      const scheduleMachineHost = () => {
        ...
        hostTail = hostTail
          ...
          .catch((error) => {
            console.error("[sandbox-agent] machine host failed", error);
          });
      };
      unsubscribeMachine = machine.subscribe(scheduleMachineHost);
      ...
      const applyRoomIoParticipant = () => {
        ...
        console.log(`[sandbox-agent] voice participant ${identity ?? "<auto>"}`);
      };
      ...
      ctx.room.on(RoomEvent.LocalTrackPublished, (publication) => {
        console.log(
          `[sandbox-agent] local track published sid=${String(publication.sid ?? "<none>")} source=${String(publication.source ?? "<unknown>")}`,
        );
      });
      ctx.room.on(RoomEvent.LocalTrackSubscribed, (track) => {
        console.log(`[sandbox-agent] local track subscribed sid=${String(track.sid ?? "<none>")}`);
      });
      ...
      session.on(voice.AgentSessionEventTypes.AgentStateChanged, (event) => {
        console.log(`[sandbox-agent] state ${event.oldState} -> ${event.newState}`);
      });
      session.on(voice.AgentSessionEventTypes.SpeechCreated, (event) => {
        const speechId = readSpeechHandleId(event);
        console.log(`[sandbox-agent] speech created ${speechId ?? "<unknown>"} source=${event.source}`);
      });
      session.on(voice.AgentSessionEventTypes.FunctionToolsExecuted, (event) => {
        console.log(`[sandbox-agent] function tools executed ${summarizeLiveKitFunctionToolsExecuted(event)}`);
      });
      session.on(voice.AgentSessionEventTypes.Error, (event) => {
        console.error("[sandbox-agent] LiveKit session error", event.error);
      });
      ...
      cameraSampler = attachCameraSampler({
        ...
        onImage: (image) => {
          ...
          void syncMachineRuntime(machine, {
            ...
            console.warn("[sandbox-agent] failed to sync camera sensor", error);
          });
        },
      });
      ...
      scheduleMachineHost();
      console.log(`[sandbox-agent] voice session started for ${roomName}; liveMode=${isLiveMode()}`);
      console.log(`[sandbox-agent] connected to ${roomName} with ${liveKitExecutor.connection.inference?.tools.length ?? 0} projected tools`);
    } catch (error) {
      await stopCameraSampler().catch((stopError) => {
        console.warn("[sandbox-agent] failed to stop camera sampler", stopError);
      });
      releaseLease();
      throw error;
    }
...
function attachRealtimeDebugLogging(agent: SandboxVoiceAgent): void {
  ...
  if (!realtimeSession?.on) {
    console.log("[sandbox-agent] realtime debug logging skipped; no realtime session");
    return;
  }
  ...
  realtimeSession.on("generation_created", (event) => {
    ...
    console.log(
      `[sandbox-agent] realtime generation_created responseId=${String(record.responseId ?? "<none>")} userInitiated=${String(record.userInitiated ?? "<unknown>")}`,
    );
  });
  realtimeSession.on("input_speech_started", () => {
    console.warn("[sandbox-agent] realtime input_speech_started");
  });
  realtimeSession.on("input_speech_stopped", (event) => {
    const record = asRecord(event);
    console.warn(
      `[sandbox-agent] realtime input_speech_stopped userTranscriptionEnabled=${String(record.userTranscriptionEnabled ?? "<unknown>")}`,
    );
  });
  realtimeSession.on("input_audio_transcription_completed", (event) => {
    ...
    console.log(
      `[sandbox-agent] realtime input_audio_transcription_completed itemId=${String(record.itemId ?? "<none>")} chars=${transcript.length}`,
    );
  });
  realtimeSession.on("openai_client_event_queued", (event) => {
    const summary = summarizeRealtimeClientEvent(event);
    if (summary) console.log(`[sandbox-agent] openai client ${summary}`);
  });
  realtimeSession.on("openai_server_event_received", (event) => {
    const summary = summarizeRealtimeServerEvent(event);
    if (summary) console.log(`[sandbox-agent] openai server ${summary}`);
  });
  realtimeSession.on("metrics_collected", (metrics) => {
    ...
    console.log(`[sandbox-agent] realtime metrics_collected type=${type ?? "<unknown>"}`);
  });
}
...
function parseLiveKitSandboxMessage(payload: Uint8Array): string | FrameDraft<any> | undefined {
  try {
    ...
    if (DEBUG_REALTIME_EVENTS) {
      console.log(
        `[sandbox-agent] livekit data message parsed content_chars=${content.length} attachments=${summarizeSandboxAttachments(attachments)}`,
      );
    }
    ...
    if (DEBUG_REALTIME_EVENTS) {
      console.warn("[sandbox-agent] failed to parse livekit data message", error);
    }
    return undefined;
  }
}

function summarizeSandboxAttachments(attachments: readonly SandboxAttachmentData[]): string {
...
function normalizeLiveKitAttachments(value: unknown): SandboxAttachmentData[] {
  if (!Array.isArray(value)) return [];
  const attachments: SandboxAttachmentData[] = [];
