...
The agent name is `sandbox-agent`; `apps/sandbox/convex/livekitAgentActions.ts` dispatches that exact name.
...
Then run the sandbox app and enable `voice` in the terminal toolbar. The browser will request a LiveKit token from Convex, join the room, and dispatch this worker.
