...
export const sendMessage = action({
  ...
  handler: async (ctx, { sessionId, content, attachments }): Promise<{ success: true }> => {
    ...
    const rootInstance = hydrateSandboxInstance(session.instance, sessionId);
    const contextFrames = (await ctx.runQuery(api.sessions.listMachineContextFrames, { sessionId })) as Frame<SandboxAttachmentData>[];
    ...
    const executor: Executor<SandboxAttachmentData> = {
      run: (request) =>
    ...
    const machine = createMachine<SandboxAttachmentData>({
      id: sessionId,
    ...
    const producedFrames: Frame<SandboxAttachmentData>[] = [];
...
type DurableFrame = Frame<SandboxAttachmentData> & { metadata?: Record<string, unknown> };

function prepareMachineFrame(frame: Frame<SandboxAttachmentData>, rootGeneratorId: string): DurableFrame {
  const mode =
...
async function persistFrameMessages(
  ctx: ActionCtx,
  {
    ...
    frame: Frame<SandboxAttachmentData>;
    frameId: Id<"frames">;
...
function shouldPersistAssistantMessage(
  frame: Frame<SandboxAttachmentData>,
  message: Frame<SandboxAttachmentData>["messages"][number],
  rootGeneratorId: string,
): boolean {
