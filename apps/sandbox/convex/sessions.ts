...
import {
  createSandboxClientSnapshot,
  createSandboxCharter,
  hydrateSandboxInstance,
  hydrateSandboxSourceInstance,
  serializeSandboxSourceInstance,
} from "@projectors/sandbox-agent/src/projector-sandbox.js";
import {
...
import type { SandboxAttachmentData } from "@projectors/sandbox-agent/src/attachments.js";
...
export const get = query({
  ...
  handler: async (ctx, { id, timetravelFrameId }) => {
    ...
    const clientSnapshot = createSandboxClientSnapshot(instance, id, syncState);
...
type ObservabilityState = {
  root: ReturnType<typeof hydrateSandboxInstance>;
  charter: Charter<SandboxAttachmentData>;
  frames: Frame<SandboxAttachmentData>[];
  executor: Executor<SandboxAttachmentData>;
...
function collectRuntimeInspectionTargets(
  nodes: CompiledContributor<SandboxAttachmentData>[],
): RuntimeInspectionTarget[] {
  const targets: RuntimeInspectionTarget[] = [];
  const visit = (node: CompiledContributor<SandboxAttachmentData>) => {
    targets.push({
      generatorId: node.id,
      kind: node.kind,
...
): CompiledInference<SandboxAttachmentData> {
...
function createObservabilityExecutor(): Executor<SandboxAttachmentData> {
  const discreteExecutor = new AiSdkExecutor<SandboxAttachmentData>({
...
async function applyFrameInstanceMessage(
  ...
  applyInstanceMessage(source, message, createSandboxCharter());
  ...
  await ctx.db.insert("projectorInstanceLog", {
    ...
    instance: escapeConvexJson(serializeSandboxSourceInstance(source)),
    createdAt: Date.now(),
  });
}
...
async function getLatestSourceContainingInstance(
  ...
  for (const log of latestLogs) {
    const source = hydrateSandboxSourceInstance(restoreConvexJson(log.instance));
...
async function getLatestSource(
  ...
  return latestLog ? hydrateSandboxSourceInstance(restoreConvexJson(latestLog.instance)) : null;
}
...
function containsInstance(instance: ReturnType<typeof hydrateSandboxSourceInstance>, targetInstanceId: string): boolean {
