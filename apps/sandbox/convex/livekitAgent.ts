...
import { createSandboxClientSnapshot } from "@projectors/sandbox-agent/src/projector-sandbox.js";
import type { MachineSyncState } from "@projectors/core/client";
import { collectSessionFramePath, getLatestSessionFrameDoc } from "./frameHistory";
...
export const getAgentInit = query({
  ...
  handler: async (ctx, { roomName }) => {
    ...
    const latestFrame = await getLatestSessionFrameDoc(ctx, room.sessionId);
    ...
    const logs = await ctx.db
      ...
      .collect();
    const framePath = await collectSessionFramePath(ctx, session, latestFrame._id);
    ...
    return {
      ...
      instance,
      clientSnapshot: stripClientSchemas(createSandboxClientSnapshot(instance, room.sessionId, syncState)),
      syncState,
