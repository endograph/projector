...
import type { SandboxAttachment, SandboxClientInstance, SandboxClientSnapshot, SandboxMessage } from "@/src/types/display";
...
export function HomeClient({ initialSessionId }: { initialSessionId: Id<"sessions"> | null }) {
  ...
  return (
    <ProjectorProvider
      ...
      snapshot={session?.clientSnapshot as SandboxClientSnapshot | undefined}
...
function HomeClientContent({
  ...
  resolveAttachmentUrls: (args: { attachments: StoredAttachmentInput[] }) => Promise<SandboxAttachment[]>;
  cloneFromFrame: (args: {
  ...
  serverMessages: SandboxMessage[] | undefined;
  liveKitWorkerStatus:
    | {
        status: string;
        ready: boolean;
  ...
  const [sendLiveKitMessage, setSendLiveKitMessage] = useState<((message: { content: string; attachments: SandboxAttachment[] }) => Promise<void>) | null>(null);
  ...
  const sortedMessages = ((serverMessages ?? []) as SandboxMessage[])
    .slice()
  ...
  const handleLiveKitSenderChange = useCallback((sender: ((message: { content: string; attachments: SandboxAttachment[] }) => Promise<void>) | null) => {
...
type PendingAttachment = SandboxAttachment & {
  previewUrl?: string;
};

type StoredAttachmentInput = Omit<SandboxAttachment, "url" | "storageId" | "dataUrl"> & {
  storageId: Id<"_storage">;
};

function toStoredAttachmentInput(attachment: SandboxAttachment): StoredAttachmentInput {
  return {
...
function getAgentControlsState(instances: SandboxClientInstance[]) {
...
function getThemeHue(instances: SandboxClientInstance[]): number {
  const state = findState(instances, "sandbox")?.value as
    | { themeHue?: number }
    | undefined;
...
function findState(instances: SandboxClientInstance[], key: string) {
...
function findStateInInstance(instance: SandboxClientInstance, key: string): SandboxClientInstance["states"][number] | undefined {
  const state = instance.states.find(
    (item: SandboxClientInstance["states"][number]) => item.key === key,
  );
  if (state) return state;
