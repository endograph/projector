...
import type { SandboxAttachment, SandboxClientInstance, SandboxMessage } from "@/src/types/display";
  ...
  function TerminalPane(
    ...
    const runToggle = async (name: string, enabled: boolean) => {
      ...
      const command = effigy.getCommand(name as never, {
        ...
        optimistic: (ctx: OptimisticContext<SandboxClientInstance[]>) => {
            ...
            {messages.length === 0 ? (
              <div className="max-w-xl space-y-3 text-sm leading-6 text-terminal-green-dim">
                <p>
                  ...
                  <span className="text-terminal-green">remember this sandbox uses projector</span>.
                </p>
                {connectionError && (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <textarea
                ...
                placeholder="Message the projector sandbox..."
              />
...
function TerminalMessage({ message }: { message: SandboxMessage }) {
...
type TerminalAttachment = SandboxAttachment & {
  previewUrl?: string;
};
...
function SentAttachmentPreview({ attachment }: { attachment: SandboxAttachment }) {
...
function findCommand(instances: SandboxClientInstance[], name: string) {
  for (const instance of instances) {
...
function findCommandInInstance(instance: SandboxClientInstance, name: string): SandboxClientInstance["commands"][number] | undefined {
  const command = instance.commands.find(
    (item: SandboxClientInstance["commands"][number]) => item.name === name,
  );
  if (command) return command;
