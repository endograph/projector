...
function TreeTab({
  ...
  sessionId: Id<"sessions"> | null;
  instances: SandboxClientInstance[];
...
function InstanceTree({ instances }: { instances: SandboxClientInstance[] }) {
...
function InstanceNode({
  ...
  instance: SandboxClientInstance;
  depth: number;
  defaultExpanded?: boolean;
}) {
  const [expanded, setExpanded] = useState(defaultExpanded);
...
function StateTab({ instances }: { instances: SandboxClientInstance[] }) {
...
function collectStates(instances: SandboxClientInstance[]) {
  const states: SandboxClientInstance["states"] = [];
  const visit = (instance: SandboxClientInstance) => {
...
function CommandsTab({
  ...
  instances: SandboxClientInstance[];
  effigy: OptimisticEffigy<SandboxClientInstance[]>;
  readOnly: boolean;
}) {
  const commands = collectCommands(instances);
  const sandboxAddress = instances[0]?.states.find(
    (item) => item.key === "sandbox",
  )?.address;
  const run = async (commandMeta: SandboxClientInstance["commands"][number]) => {
    if (readOnly) return;
    ...
    const command = effigy.getCommand(name as never, {
      ...
      ...(name === "setThemeHue"
        ? {
            optimistic: (ctx) => {
              if (sandboxAddress && "hue" in input) {
                ctx.patchAt(sandboxAddress, { themeHue: input.hue });
              }
            },
          }
        : {}),
...
function PlaygroundTab({
  ...
  instances: SandboxClientInstance[];
  canonicalInstances: SandboxClientInstance[];
  effigy: OptimisticEffigy<SandboxClientInstance[]>;
  readOnly: boolean;
}) {
...
function collectCommands(instances: SandboxClientInstance[]) {
  const commands: SandboxClientInstance["commands"] = [];
  const visit = (instance: SandboxClientInstance) => {
...
function findState(
  instances: SandboxClientInstance[],
  key: string,
): SandboxClientInstance["states"][number] | undefined {
...
function findCommand(
  instances: SandboxClientInstance[],
  name: string,
): SandboxClientInstance["commands"][number] | undefined {
  for (const instance of instances) {
