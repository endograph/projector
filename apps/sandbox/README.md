...
Projector sandbox app with text chat, projected state/commands, and LiveKit voice mode.
...
Create `apps/sandbox/.env.local` with Convex and LiveKit values:
...
Voice mode also requires the worker from `apps/sandbox-agent` to be running. See `apps/sandbox-agent/README.md`.
