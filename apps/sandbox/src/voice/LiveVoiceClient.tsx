...
import type { SandboxAttachment } from "@/src/types/display";

type VoiceStatus = "idle" | "connecting" | "connected" | "disconnected" | "error";
type SendLiveKitMessage = (message: { content: string; attachments: SandboxAttachment[] }) => Promise<void>;
...
export function LiveVoiceClient({
  ...
  const attachAgentAudioPublication = useCallback((publication: RemoteTrackPublication) => {
    ...
    void audio.play().catch((error) => {
      console.info("[sandbox/livekit] unable to start agent audio playback", error);
    });
  }, []);
  ...
  const publishSender = useCallback((room: Room | null) => {
    ...
    onSendMessageChange?.(async ({ content, attachments }) => {
      ...
      console.info("[sandbox/livekit] publish text message", {
        topic: MESSAGE_TOPIC,
...
function summarizeLiveKitAttachments(attachments: readonly SandboxAttachment[]): unknown {
