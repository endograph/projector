# Sandbox App Learnings

Messy append-only notes extracted from recent sessions around `apps/sandbox`,
`apps/sandbox-agent`, LiveKit, streaming, voice, and durable projector state.
...
- Convex `$schema` failures during sandbox session creation.
- Sandbox DB refactor from turns/steps to frames.
- AI SDK executor and sandbox-agent integration.
...
The sandbox should treat Convex as the durable source of truth. LiveKit rooms,
...
The old `machineTurns` / `machineSteps` vocabulary was removed from the sandbox.
...
Current helper lives in `apps/sandbox/convex/convexJson.ts`.
...
example `node:sandboxRoot`, so they do not carry full JSON Schema by accident.
...
- Browser publishes reliable data on topic `sandbox.message.v1`.
- Payload is JSON like `{ content, sentAt }`.
- `apps/sandbox-agent` listens for `RoomEvent.DataReceived`.
...
- `getToken` / `ensureAgentDispatched` use stable room name `sandbox-${sessionId}`.
...
Final direction: keep stable room names (`sandbox-${sessionId}`) and fix the actual
...
`apps/sandbox-agent`, the new worker heard the user and got realtime transcripts /
...
Important code comment from `apps/sandbox-agent/src/agent.ts`:
...
at the time. The sandbox-agent path is where streaming was fixed first.
...
The sandbox-agent was too twitchy. Typing could trigger speech detection.
...
Local machine had Convex CLI/auth for the sandbox deployment.

Useful debugging commands from `apps/sandbox`:
...
bun run --filter @projectors/sandbox-agent build
...
ESLint note: at one point `apps/sandbox` had ESLint 9 but no flat
...
1. Start sandbox app and `apps/sandbox-agent`.
2. Create a session.
...
7. Kill `apps/sandbox-agent`.
8. Restart `apps/sandbox-agent`.
