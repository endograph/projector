...
export function assistantMessageFromTextOutput<TDataContent = never>(
  ...
  if (mapTextBlock) {
    ...
    const content = schema ? normalizeSchema(schema).parse(mapped) : mapped;
    return {
      type: "assistant",
  ...
  if (schema) {
    return {
      type: "assistant",
      content: [dataContent(normalizeSchema(schema).parse(text))],
      text,
      ...(output?.audience ? { audience: output.audience } : {}),
    };
  }
...
function isWorkCompletionReason(value: unknown): value is WorkCompletionReason {
  return value === "end-turn" || value === "done" || value === "continued" || value === "cancelled" || value === "delegated" || value === "error" || value === "terminal-action" || value === "absorbed" || value === "suppressed";
}
