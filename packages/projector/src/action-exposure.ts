 ...
 * compiled availability note promises tool search. Hidden contributions never
 * reach the compiled tool surface, so executors only ever see native or
 * deferred here.
 */
export function actionExposure(action: AnyAction): Exposure {
  const exposure = (action as { [ACTION_EXPOSURE]?: unknown })[ACTION_EXPOSURE];
  return exposure === "deferred" || exposure === "hidden" ? exposure : "native";
}
